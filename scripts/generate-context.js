...
w("# Diet Specifix - Project Context File");
w();
...
w("> 🔗 **Live App:** [https://diet-specifix.vercel.app/](https://diet-specifix.vercel.app/)");
w();
w("---");
w();
w("## 1. Project Overview");
w();
w("**Diet Specifix** is a React-based SPA for Indian diet planning. Users build meals in grams, convert to exchange-style categories, and score dietary patterns with transparent reasons.");
w();
w("### Key Features");
w("- Meal Builder (Breakfast/Lunch/Dinner/Snacks)");
w("- Scoring Engine (0-100 with reasons)");
w("- Food Explorer (Google-style search with nutrient-ranked results)");
w("- Pre-saved Plans (5 read-only templates)");
w("- User Plans (create, name, edit, reset, delete — persisted to localStorage)");
...
w("- Client-side Routing (react-router-dom)");
w("- Code Splitting (React.lazy)");
w("- Error Boundary (graceful crash recovery)");
w("- Unit Tests (Vitest)");
w();
w("---");
w();
...
w("| Routing | React Router DOM | " + (pkg.dependencies["react-router-dom"] || "?") + " |");
...
w("| Testing | Vitest | " + (pkg.devDependencies.vitest || "?") + " |");
...
w("## 5. Routing");
w();
w("| Path | Component | Lazy |");
w("|------|-----------|------|");
w("| `/` | WelcomePage | ✅ |");
w("| `/dashboard` | DashboardPage | ✅ |");
w("| `/foods` | FoodSearchPage | ✅ |");
w("| `/profile` | ProfilePage | ✅ |");
w("| `*` | Redirect → `/` | — |");
w();
w("---");
w();
...
w("```");
w();
w("### Supabase Tables & Views");
w("health_goals, user_profile_health_goals, user_profiles, major_groups, food_items, nutrient_groups, nutrient_definitions, food_nutrient_values, food_search_view");
w();
w("### Supabase RPCs");
w("- `search_foods(search_text)` — full-text food search returning food_id, food_code, food_name, food_group");
w();
w("---");
w();
w("## 7. Scoring Logic");
...
w("---");
w();
w("## 9. State Management");
w();
w("- **App.jsx** — profile (activity, goal, dietType, sex, bmiTarget)");
w("- **DashboardPage** — presetPlans(5), userPlans (localStorage), planView, activePlanId, selectedMeal/FoodId/grams/day/instructions, vegetableTarget, sugarLimit");
w("- **FoodSearchPage** — query, results, selectedFood, nutrients, activeFilter, isNutrientSearch");
w();
w("---");
w();
w("## 10. Food Explorer");
w();
w("- Debounced search (300ms, min 2 chars)");
w("- Checks if query matches a nutrient name → returns foods sorted by that nutrient value DESC");
w("- Falls back to RPC `search_foods()` for food name matching");
w("- Displays nutrient detail panel grouped by nutrient_group");
w("- Keyboard navigation (Arrow keys + Enter)");
w("- Filter chips: All / Foods / Groups");
w();
w("---");
w();
w("## 11. Persistence");
w();
w("- **User plans** → `localStorage` key: `diet-specifix-user-plans`");
w("- **Health goals** → Supabase `user_profile_health_goals`");
w("- **Auth session** → Supabase session storage");
w();
w("---");
w();
w("## 12. Pre-saved Plans");
w();
...
w("## 16. Testing");
w();
w("- **Framework**: Vitest (" + (pkg.devDependencies.vitest || "?") + ")");
w("- **Tests**: `tests/scoringEngine.test.js`, `tests/nutrientEngine.test.js`");
w("- **Run**: `npm test` or `npm run test:watch`");
w();
w("---");
w();
w("## 17. Deployment");
w();
w("- **Live URL**: [https://diet-specifix.vercel.app/](https://diet-specifix.vercel.app/)");
w("- **Platform**: Vercel (auto-deploys from `main` branch)");
w("- Build: `npm run build` → outputs to `dist/`");
w("- SPA routing handled by `vercel.json` rewrites");
w("- Install uses `yarn install --no-lockfile` (configured in `vercel.json`)");
w("- Environment variables set in Vercel Project Settings");
w();
w("---");
w();
w("## 18. Design Decisions");
w();
w("- No TypeScript — plain JSX for speed");
w("- Modular architecture — pages, UI components, engines separated");
w("- React Router for real URLs + code splitting via React.lazy");
w("- Error Boundary wraps all routes");
w("- Preset plans read-only; edits auto-copy to user plan");
w("- Indian exchange system (grams / gramsPerExchange)");
w("- Transparent scoring with reasons array");
w("- Supabase for auth + health goals + food database");
w("- " + foodIds.length + " local foods for offline meal builder (expandable via DB)");
w("- localStorage for plan persistence (Supabase persistence planned)");
w("- Nutrient-aware search: typing a nutrient name ranks foods by that nutrient");
w();
w("---");
w();
w("## 19. Future");
w();
w("- Persist plans to Supabase");
w("- Weekly planner view");
w("- Recharts nutrient visualizations");
w("- PDF export");
w("- Mobile UX improvements");
w();
